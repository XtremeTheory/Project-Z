  ...
  global $adminpath;
    ...
    if($userTier < $rqstTier) {
      ...
      $_SESSION['sessExpired'] = TRUE;
  		header("Location:".$adminpath."login.php?location=" . urlencode($_SERVER['REQUEST_URI']));
  		exit();
		}
    ...
    mysqli_close($test_db);
    $_SESSION['sessExpired'] = TRUE;
    header("Location:".$adminpath."login.php?location=" . $plocation);
    exit();
  }
}

function verifyDasher($pageName) {
	global $path;
  global $dasherpath;
	global $test_db;
	global $timestamp;
    ...
    if($dasherID == 0) {
      ...
      logActivity("10",$uid,$pageName);
  		...
  		mysqli_close($test_db);
      $_SESSION['sessExpired'] = TRUE;
  		header("Location:".$dasherpath."login.php?location=" . urlencode($_SERVER['REQUEST_URI']));
  		exit();
		}
    ...
    mysqli_close($test_db);
    $_SESSION['sessExpired'] = TRUE;
    header("Location:".$dasherpath."login.php?location=" . $plocation);
    exit();
  }
}
